...
fileRead = 'LRE' #change variable value, possible values: LPE, LRE, LPI, LRI
...
def readFolder(folder, return_dict, notes_dict):
    ...
    for sub in subfolders:
            ...
            if ('.xls' in file):
                try:
                    if (fileRead in ['LRI', 'LPI']):
                        data = pd.read_excel(file, header=3, sheet_name=None)
                    else:
                    ...
                    for sheet in sheets:
                        dataa = data[sheet]
                        thisColumns = list(dataa.columns.values)
                        if (thisColumns[0] != 'No.'):
                            dataa = skipEmptyRow(dataa, replacement)
                        if (len(thisColumns) == replacementLen):
                            dataa.columns = replacement.columns
                            dataa = dataa.drop(columns=['No.'])
                            if (fileRead in ['LRE', 'LPE']):
                                allData = dataa[dataa['Cakupan Kegiatan'].notnull()]
                            elif (fileRead in ['LRI', 'LPI']):
                                allData = dataa[dataa['Nama Kegiatan'].notnull()]
                            ...
                            if (rows > 0):
                                ...
                                if (not np.array_equal(validationColumns, thisColumns)):
                                    ...
                                    for i in range(0, rows):
                                        ...
                                        allData.loc[rowIndex,'Nama File'] = namaFile + ' - Sheet ' + sheet
        ...
        os.chdir('..')
    return_dict[folder] = template
    notes_dict[folder] = notes
    
if __name__=="__main__":
    freeze_support()
    procs = []
    names = [x for x in filter(os.path.isdir, os.listdir(os.getcwd()))]#change to the actual folders
    manager = Manager()
    return_dict = manager.dict()
    notes_dict = manager.dict()
    folders = [x for x in filter(os.path.isdir, os.listdir(os.getcwd()))]#change to the actual folders
    pool = Pool(maxtasksperchild=1)
    pool.starmap(readFolder, zip(folders, repeat(return_dict), repeat(notes_dict)))
    pool.close()
    pool.join()
    ...
    for key, value in return_dict.items():
        ...
        for i in range(0, rows):
            no += 1
            rowIndex = value.index[i]
            value.loc[rowIndex,'No.'] = str(no) + '.'
        template = pd.concat([template, value]) 
    errorNotes = []
    ...
    template.to_excel(fileRead + " Result Raw Multi.xlsx", index=False) 
    df = pd.DataFrame(np.array(errorNotes), columns=['File', 'Error'])
    df.to_excel(fileRead + ' Mistake Notes Multi.xlsx', 'Sheet1', index=False)
    print('Done reading...')
    print('Cleaning...')
    ...
    data = pd.read_excel(fileRead + ' Result Raw Multi.xlsx', na_filter=False)
    ...
    print('Exporting...')
    duplicate.to_excel(fileRead + " Result Final Multi.xlsx", index=False)