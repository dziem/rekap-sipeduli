...
template = pd.read_excel('lpiplus_template.xlsx')
replacement = pd.read_excel('lpi_replace.xlsx')
...
for folder in folders:
    ...
    for sub in subfolders:
        ...
        for file in files:
            print('--'+file)
            if ('_lpi' in file and '.xls' not in file):
            ...
            if ('.xls' in file):
                ...
                if (pos != -1):
                    ...
                    try:
                        lre = file.lower().find('_lre')
                        lpe = file.lower().find('_lpe')
                        if (lre == -1 and lpe == -1):
                            try:
                                data = pd.read_excel(file, header=3, sheet_name='Laporan Inklusi Keuangan')
                            except:
...
print('Done reading...')
print('Exporting...')
template.to_excel("LPI Plus Result.xlsx", index=False) 
with open("LPI Plus Mistake Notes.txt", "w") as txt_file:
    for line in notes:
        txt_file.write(line)
        txt_file.write("\n")
